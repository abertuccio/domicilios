...
o <- data.frame(id=c(1,2,3,4,5),nombre = c("CARLOS o MARTIN",
                           "PEPE_TAMBIEN_DENOMINADO_FRANCISCO",
                           "HAEDO_(MARIANO_J_HAEDO)",
                            "JOSE_DE_SAN_MARTIN",
                           "ESTACIóN_ATAMISQUI"), stringsAsFactors = FALSE)


library(tidyr)
o$norm <- tolower(o$nombre)
o <- o %>% separate_rows(norm, sep=" o ")
